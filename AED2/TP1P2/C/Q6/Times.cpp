/**
 * TP01P2Q06 Fila circular em C
 * 
 * @author Thiago Henrique de Castro Oliveira
 * @version 1 09/2019 Este algoritmo le paginas html e preenche uma fila de Times
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/*
* Abaixo sao realiazadas as operacoes da fila
*/
char *removerFila(Fila fila[])
{
    ...
    char *resp = fila->array[fila->primeiro].nome;
...
void inserirFila(Fila fila[], Time *time)
{
    //printf("%d", fila->ultimo);
    if (((fila->ultimo + 1) % 6) == fila->primeiro)
    {
        removerFila(fila);
    }
    fila->array[fila->ultimo] = *time;
    ...
    for (int i = 0; i < 6; i++)
    {
        if (fila->array[i].fundacaoAno != 0)
        {
            resp += fila->array[i].fundacaoAno;
            cont++;
        }
    }
    resp = resp / cont;
...
int main(int argc, char const *argv[])
{
    ...
    */
    ...
    char *newline;
    /*
    * Para cada entrada, le e inseri o Time na fila
    */
    ...
    */
    ...
    for (int i = 0; i < entry; i++)
    {
        ...
        if (ops[0] == 'I')
        {
            ...
            subString_delimiter(ops, 2, strlen(ops), path);
            correct = strchr(path, '\n');
            if (correct)
                *correct = 0;
            time = (Time *)malloc(sizeof(Time));
