/**
 * TP01P2Q21 Heapsort Parcial em C
 * 
...
int constroi(int tamHeap, Lista lista[])
{
    int comp = 0;
    for (int i = tamHeap; i > 1 && strcmp(lista->array[i].estadio, lista->array[i / 2].estadio) > 0; i /= 2)
    {
        comp++;
        swap(i, i / 2, lista);
    }
...
int reconstroi(Lista lista[], int tamHeap)
{
    int i = 1, filho;
    int comp = 0;
    while (i <= (tamHeap / 2))
    {
        ...
        if (strcmp(lista->array[2 * i].estadio, lista->array[2 * i + 1].estadio) > 0 || 2 * i == tamHeap)
        {
            filho = 2 * i;
            comp++;
        }
        ...
        if (strcmp(lista->array[i].estadio, lista->array[filho].estadio) < 0)
        {
            comp++;
            swap(i, filho, lista);
            i = filho;
...
int heapsort(Lista lista[])
{
    int comp = 0;
    //Alterar o vetor ignorando a posicao zero
    for (int i = lista->n; i > 0; i--)
    {
        memccpy(&(lista->array[i]), &(lista->array[i - 1]), sizeof(Time), sizeof(Time));
    }

    //Contrucao do heap
    for (int tamHeap = 1; tamHeap <= lista->n; tamHeap++)
    {
        comp += constroi(tamHeap, lista);
    }
    ...
    while (tamHeap > 1)
    {
        swap(1, tamHeap--, lista);
        comp += reconstroi(lista, tamHeap);
    }
...
int main(int argc, char const *argv[])
{
    ...
    */
    ...
    int comp = heapsort(lista);
    double total = (clock() - comeco) / (double)CLOCKS_PER_SEC / 1000;

    FILE *log = fopen("649884_heapsort.txt", "w");
