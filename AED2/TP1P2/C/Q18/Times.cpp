...
void radixSort(Lista lista[])
{
    ...
    // loop ate encontrar o maior digito significante
    while (largestNum / significantDigit > 0)
    {

        int bucket[10] = {0};

        // conta o numero de chaves ou digitos que vao para cada bucket
        for (i = 0; i < lista->n; i++)
            bucket[(lista->array[i].paginaTam / significantDigit) % 10]++;

        /**
     * adiciona a conta das buckets anteriores,
     * */
        for (i = 1; i < 10; i++)
            bucket[i] += bucket[i - 1];

        // utiliza a bucket para preencher um array semi ordenado
        ...
        // vai para o proximo digito significante
        significantDigit *= 10;
    }
    free(semiSorted);
}
...
int main(int argc, char const *argv[])
{
    ...
    */
    ...
    FILE *log = fopen("649884_radixsort.txt", "w");
    fprintf(log, "649884\t%f\t%d", total, 0);
    fclose(log);
