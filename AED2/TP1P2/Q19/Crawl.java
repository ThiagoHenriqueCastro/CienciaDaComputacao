...
/**
 * TP01P2Q19 Selecao Parcial em Java
 * 
 * @author Thiago Henrique de Castro Oliveira
 * @version 1 09/2019 Este algoritmo le paginas html e as ordena em uma lista
 */
...
class Lista {
    ...
    public Integer[] quicksort(int esq, int dir) {
        int comparisons = 0;
        int moves = 0;
        long start = System.currentTimeMillis();
        ...
        int i = esq;
        int j = dir;
        Time pivo = array[(dir + esq) / 2].clone();
        while (i <= j) {
            while (array[i].getApelido().compareTo(pivo.getApelido()) < 0)
                i++;
            while (array[j].getApelido().compareTo(pivo.getApelido()) > 0)
                j++;
...
public class Crawl {
    public static void main(String[] args) throws Exception {
        ...
        logs = lista.quicksort(0, 10);
        try {
            FileWriter writer = new FileWriter("649884_quicksort.txt");
