...
class Time {
    ...
    /**
     * le um arquivo html e retorna dados do mesmo
     */
...
class Lista {
    ...
    public Time[] mergeSort(Time[] arr, int len) {

        if (arr.length <= 1) {
            return array;
        }

        Time[] first = new Time[len / 2];
        Time[] second = new Time[len - first.length];
    ...
    public void merge(Time[] first, Time[] second, Time[] result) {
        int indexFirst = 0;
        int indexSecond = 0;
        int indexMerged = 0;

        while (indexFirst < first.length && indexSecond < second.length) {
            if (first[indexFirst].getnomeAquivo().compareTo(second[indexSecond].getnomeAquivo()) < 0) {
                result[indexMerged] = first[indexFirst++].clone();
            } else {
                result[indexMerged] = second[indexSecond++].clone();
            }
            indexMerged++;
        }
...
public class Crawl {
    public static void main(String[] args) throws Exception {
        ...
        lista.mergeSort(lista.array, lista.n);
