...
class _Arvore {
    ...
    public void inserir(Time x) {
        raiz = inserir(raiz, x);
    }

    public No inserir(No i, Time x) {
        // se a arvore do fora estiver vazia, nao posso inserir na arvore de dentro
        // ainda
        if (i == null) {
            // MyIO.println("Erro ao inserir! Estrutura exterior vazia");
        } else if ((x.getFundacaoAno() % 15) < i.elemento)
            i.esq = inserir(i.esq, x);
        else if ((x.getFundacaoAno() % 15) > i.elemento)
            i.dir = inserir(i.dir, x);
        else
            i.raiz = inserir(i.raiz, x);

        return i;
    }

    public No_2 inserir(No_2 i, Time x) {
        if (i == null)
            i = new No_2(x);
    ...
    private boolean pesquisar(No_2 i, String x) {
        boolean resp;
        if (i == null) {
