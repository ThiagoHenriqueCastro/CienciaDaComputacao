...
typedef struct No
{
    ...
    struct No *esq; // Aponta a esq
    struct No *dir; // Aponta a dir
    int nivel;
} No;
...
No *novoNo(Time time[])
{
    ...
    memccpy(&nova->elemento, time, sizeof(Time), sizeof(Time));
...
No *_inserir(No *i, Time x[])
{
    if (i == NULL)
    {
        ...
        i->esq = _inserir(i->esq, x);
    else if (strcmp(x->nome, i->elemento.nome) > 0)
        i->dir = _inserir(i->dir, x);
    else
    {
...
void inserir(Time x[])
{
    raiz = _inserir(raiz, x);
}

boolean pesquisarRec(No *i, char *x)
{
    boolean resp;
    ...
    if (i == NULL)
        ...
        i->esq = pesquisarRec(i->esq, x);
    else
        i->dir = pesquisarRec(i->dir, x);

    return resp;
}

boolean pesquisar(char *x)
{
...
int main(int argc, char const *argv[])
{
    ...
    */
    ...
    char *newline;
    /*
    * Para cada entrada, le e insere o Time na arvore
    */
    ...
    char searches[ENTRY][LINE];
    int searchIndex = 0;

    /*
    * Preenche um vetor com as pesquisar
    */
    do
    {

        fgets(searches[searchIndex], LINE, stdin);
    ...
    {
        new = strchr(searches[i], '\n');
        if (new)
            *new = 0;
        printf("%s\n", pesquisar(searches[i]) ? "SIM" : "NÃO");
    }
